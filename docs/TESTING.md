...
| T0.4 | Each relay board | Toggle input from Mega, listen + meter the contacts | Clicks; NO closes, NC opens; coil drawn from 5V rail |
| T0.5 | Each motor | 12 V direct, uncoupled then coupled | 16 RPM; self-locks when de-energized; no grinding |
| T0.6 | Heater + blower | 12 V direct for 30 s on ceramic tile | Warm air; blower runs; PTC case intact |
...
| T1.3 | Heater loop | Command heater ON 60 s via firmware | Relay duty audible; heater branch 8.3 A ±10 %; blower on |
...
|---|---|---|---|---|
| T3.1 | 1 umbrella, light moisture | Cycle time | ≤ ~60 min |  |
| T3.2 | 3 umbrellas, light moisture | Cycle time | ≈ 50–70 min |  |
| T3.3 | 3 umbrellas, soaked | Cycle time | ≈ 2–2.5 h |  |
| T3.4 | 3-umbrella cycle | **Energy** | **≈ 90–95 Wh** |  |
| T3.5 | Repeated cycles on one charge | Cycle count | 3–4 full cycles |  |
| T3.6 | Mid-cycle | Chamber air temp | 40–60 °C |  |
| T3.7 | Result | Canopy condition | Dry to touch, no fabric odor/deformation |  |
| T3.8 | Steady draw | Battery current | ≤ 12.5 A (heater + 3 motors) |  |
| T3.9 | Station independence | Jam one umbrella mid-cycle | Other stations + heater continue; only that 3A fuse opens if stalled |  |

## 6. T4 — Safety drills (all must pass before unattended use)

| ID | Drill | Procedure | Pass criteria |
|---|---|---|---|
| T4.1 | Over-temp cutoff | Heat chamber; temporarily lower `T_CUT` to 45 °C | Heater relay drops; latched until `T < T_RESET`; stations/fan keep running |
| T4.2 | Sensor loss | Unplug DS18B20 mid-cycle (3-fail logic) | FAULT state; heater OFF; red LED + buzzer |
| T4.3 | Fail-short relay | Simulate welded contact (jumper relay output) | Rocker switch kills the branch; 15 A fuse opens on sustained short |
